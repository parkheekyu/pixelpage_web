      ...
      <section className="py-28 lg:py-36 bg-surface-white border-t border-border">
        <div className="max-w-[1240px] mx-auto px-6 lg:px-12">
          ...
          <Reveal className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: "🎯", title: "Intangible First", body: "무형 서비스 전담. 다른 의사결정 구조를 이해합니다." },
              { icon: "✍️", title: "Narrative Creative", body: "브랜드 톤을 해치지 않는 서사를 설계합니다." },
              { icon: "⚡", title: "Short-Cycle Wins", body: "짧은 사이클 안에 유의미한 결과를 만듭니다." },
              { icon: "🏆", title: "Category Leader", body: "함께한 브랜드들은 카테고리 상위권에 올라섰습니다." },
            ].map(p => (
              <div key={p.title} className="rounded-xl bg-background p-8 text-center hover:shadow-card-hover transition-shadow">
                <span className="text-[36px] block mb-5">{p.icon}</span>
                <h3 className="text-[17px] font-medium text-foreground mb-2">{p.title}</h3>
                <p className="text-[15px] text-muted-foreground leading-[1.8]">{p.body}</p>
              </div>
            ))}
          </Reveal>
        </div>
